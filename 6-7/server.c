...
void send_message_to_observer(char* message) {
    if (write(observer_socket, message, strlen(message)) == -1) {
...
int send_instructions_to_clients() {
    ...
    for (int i = 0; i < CLIENTS; i++) {
        ...
        send_message_to_observer(observer_message);
    }
...
int main(int argc, char* argv[]) {
    ...
    if (listen(server_socket, CLIENTS + 1) == -1) {
    ...
    while ((num_clients < CLIENTS) || !num_observers) {
